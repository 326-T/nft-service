...
class ApplicantServiceTest {
      ...
      void canFindAll() {
        ...
        StepVerifier.create(actual)
            .assertNext(applicant -> assertThat(applicant)
                ...
                .containsExactly("1", "太郎", "山田", "xxx@example.org", "090-1234-5678",
                    "東京都渋谷区",
                    ""))
            .assertNext(applicant -> assertThat(applicant)
                .extracting(Applicant::getId, Applicant::getFirstName, Applicant::getLastName,
                    Applicant::getEmail, Applicant::getPhone, Applicant::getAddress,
                    Applicant::getPasswordDigest)
                .containsExactly("2", "次郎", "鈴木", "yyy@example.org", "090-9876-5432",
                    "東京都新宿区",
                    ""))
            .assertNext(applicant -> assertThat(applicant)
                .extracting(Applicant::getId, Applicant::getFirstName, Applicant::getLastName,
                    Applicant::getEmail, Applicant::getPhone, Applicant::getAddress,
                    Applicant::getPasswordDigest)
                .containsExactly("3", "三郎", "佐藤", "zzz@example.org", "090-1111-2222",
                    "東京都千代田区", ""));
      }
    }
  }
    ...
    class Regular {
      ...
      void canFindById() {
        ...
        Applicant applicant1 = Applicant.builder().id("1").firstName("太郎").lastName("山田")
            .email("xxx@example.org").phone("090-1234-5678").address("東京都渋谷区")
            .passwordDigest("").build();
        when(applicantRepository.findById("1")).thenReturn(Mono.just(applicant1));
        // when
        Mono<Applicant> actual = applicantService.findById("1");
        // then
        StepVerifier.create(actual)
            .assertNext(applicant -> assertThat(applicant)
                .extracting(Applicant::getId, Applicant::getFirstName, Applicant::getLastName,
  ...
  class DeleteById {
    ...
    class Regular {
      ...
      void canDeleteById() {
        // given
        when(applicantRepository.deleteById("1")).thenReturn(Mono.empty());
        // when
        Mono<Void> actual = applicantService.deleteById("1");
        // then
